...
def test_complete_flow():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if events:
                print("\n   📅 Events for testuser:")
                                for i, event in enumerate(events, 1):                    course = event.get('course', {})                    room = event.get('room', {})                    print(f"      {i}. {event.get('title')} ({course.get('code', 'N/A')})")                    print(f"         - Room: {room.get('building', 'N/A')} {room.get('number', 'N/A')}")                    print(f"         - Lecturer: {event.get('lecturer', 'N/A')}")                    print(f"         - Type: {event.get('event_type', 'N/A')}")                    if event.get('is_urgent'):                        print(f"         - ⚠️  URGENT")
            else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   - Found {len(events)} events for testuser")
                        ...
                        # Show first 3 events            for i, event in enumerate(events[:3], 1):                course = event.get('course', {})                print(f"   {i}. {event.get('title')} - {course.get('code', 'N/A')}")
        else:
    ...
    print("- ENGGEN205 Lecture")
    print("- COMPSCI101 Lab")
    print("- STATS100 Lecture & Mid-term Exam")
    print("- BUS202 Seminar")
    print("- BIOSCI101 Lab")
